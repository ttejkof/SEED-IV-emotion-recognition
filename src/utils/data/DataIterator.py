...
class IterateHuman(IterableDataset):    
    def __init__(self, dataset, human_id):
    ...
    def __iter__(self):
        for session in self.dataset:
            for trial in session:
                if trial.human_id == self.human_id:
                    for i, (eeg_recoding, label) in enumerate(trial):
                        video_id = i
                        yield eeg_recoding, label, (session.session_id, trial.human_id, video_id)
...
class IterateAll(IterableDataset):
